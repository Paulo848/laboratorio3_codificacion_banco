...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Leídos " << lenDatos << " bytes: " ;

    unsigned char* datos_puros = new unsigned char [lenDatos];

    for(int i = 0; i < lenDatos; i++) datos_puros[i] = static_cast<unsigned char>(datos[i]);

    for(int i = 0; i < lenDatos/2; i++) std::cout << (int)datos_puros[i];

    std::cout << "\n finalizo" << std::endl;

    unsigned char* decodificado = nullptr;

    std::size_t lenDatos_ = static_cast<std::size_t>(lenDatos);

    switch (metodo) {
    case 1:
        decodificado = de_metodo_1(semilla,datos_puros,lenDatos_);
        break;
    case 2:
        decodificado = de_metodo_2(semilla,datos_puros,lenDatos_);
        break;
    default:
        break;
    }
